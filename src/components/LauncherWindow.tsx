...
// 根据插件ID获取对应的图标（模块级纯函数：引用稳定，避免击穿结果列表的 React.memo）
const getPluginIcon = (pluginId: string, className: string) => {
  switch (pluginId) {
    ...
    case "clipboard":
      return (
        <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 002 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" />
        </svg>
      );
    default:
      return (
...
export function LauncherWindow({ updateInfo }: LauncherWindowProps) {
  ...
  useEffect(() => {
    ...
    // 该 interval 仅作 onFocusChanged 的兜底（隐藏→显示的边沿检测）；页面隐藏时跳过 IPC，首次成功聚焦后自停
    ...
    const checkVisibilityAndFocus = async () => {
      ...
      try {
        ...
        if (isVisible && !lastVisibilityState && inputRef.current) {
          ...
          // 兜底目的已达成，主路径由 onFocusChanged 覆盖，停止轮询
          if (focusInterval) {
