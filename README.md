...
### v1.0.66
- 🪟 **Windows 应用与脚本启动**
  - 启动 `.bat` / `.cmd` 时使用 `cmd /c start /D <脚本目录>`，工作目录与资源管理器双击一致，批处理内相对路径（如 `start "" "同目录.exe"`）可正确解析
  - 启动 `.exe`、`.lnk` 等非批处理项时改为通过 `ShellExecuteEx`（与 `launch_file` 一致），不再经 `cmd /c start` 包装，减少子进程继承本应用环境变量，改善编辑器内置终端中 `PATH` 等与从开始菜单启动不一致的问题
  - `launch_file` 对 `.exe` / `.com` 设置 `lpDirectory` 为可执行文件所在目录，与从文件夹打开程序时的起始目录行为更接近
- 🔧 **启动器内置常用系统入口**（系统文件夹搜索，可搜中文/拼音/英文名）
  - 环境变量（`rundll32` 打开系统环境变量对话框）、高级系统设置、设备管理器、服务、网络连接、磁盘管理
  - `launch_file` 支持 `rf-builtin:` 虚拟路径；`System32` 下 `.msc` / `.cpl` 与内置项在结果展示中与原有系统文件夹一致
