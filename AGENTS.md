...
## Concurrency rules (learned from real deadlocks, see docs/DEADLOCK_DEBUGGING.md)

- **Never call Tauri window APIs (`is_visible`/`show`/`set_focus`/`hide`) inside event-loop callbacks** (tray/menu/window event handlers in `main.rs`) — they block waiting for the event loop to answer, which is the callback's own thread → instant self-deadlock. Spawn a thread instead.
- **Never do DB/network/slow work in sync `#[tauri::command] pub fn`** — sync commands run on the main (event-loop) thread; blocking there hangs every window API request. Use `async fn` + `spawn_blocking` for anything touching `SHARED_CONNECTION`.
- **Never hold a `db::get_connection` guard across `.await`, message sends, or callbacks** — `SHARED_CONNECTION` is a process-wide mutex; holding it long freezes every DB user including the main thread.
- **Never pump-skip: any thread owning a window must run a message loop** (see `everything_search.rs` IPC thread). Never `PostQuitMessage` from a window owned by a resident thread.
- **Never do slow work while holding `OpenClipboard`** — snapshot bytes, close, then process on a background thread (see `clipboard.rs` monitor).
- Hang debugging: use the dump toolchain documented in `docs/DEADLOCK_DEBUGGING.md` §2 (minidump + PDB symbolication without cdb). Debug PDB lives at `src-tauri/target/debug/re_fast.pdb`.
